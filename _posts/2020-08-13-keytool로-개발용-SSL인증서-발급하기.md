---
layout: post
title:  "keytool로 개발용 SSL인증서 발급하기"
...
Java 웹 어플리케이션을 개발 할 때, HTTPS를 적용을 해야할 때 SSL인증서를 발급해야한다.

기본적으로 SSL인증서를 발급할 때, 비용이 들게 된다. 

물론 Let's Encrypt에서 발급할 땐 무료이다. 하지만 Let's Encrypt에서 인증서를 발급하면 특정 도메인의 80포트로 접근 가능한 웹 서버가 필요하다.

상황이 여의치 않은 경우, CA의 인증을 받지 않은 말 그대로 개발용 인증서를 JDK에서 제공하는 keytool을 통하여 발급 받는 방법이 있다.
...
이렇게 발급한 해당 인증서를 Tomcat과 같은 WAS(Java Servlet Container)에 설정해주면 된다.

개발용으로 발급된 인증서이긴 하지만 Was에서 keystore 설정 할 때, 정상 인증서랑 차이는 없다.
