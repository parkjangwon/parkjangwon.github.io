...
유명한 디컴파일러로 **jd-gui**가 있다. 해당 디컴파일로의 단점으로는 **Java 1.8 이상의 버전으로 컴파일 된 소스를 디컴파일 할 수 없다는 점**이다.
...
CFR Decompiler는 Java 1.8이상의 최신 버전으로 컴파일된 class 파일을 문제 없이 java 파일로 디컴파일이 가능하다.
...
jar 파일로 배포 된다. 따라서 cfr을 사용하려면 사용할 <U>환경에 Jre & Jdk가 설치</U>되어 있어야 한다.

또한 **jar**이기 때문에, 사용자의 **OS환경과 무관**하게 java만 설치 되어있으면 사용 가능하다.
